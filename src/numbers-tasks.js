...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multiplyNums = x1 * x2 + y1 * y2;
  const magnitude1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magnitude2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(multiplyNums / (magnitude1 * magnitude2));
}
...
function getSumOfDigits(num) {
  const sum = `${num}`.split('').reduce((total, number) => total + +number, 0);
  return sum;
}
...
function getCountOfOddNumbers(number) {
  const count = Math.floor((Math.abs(number) + 1) / 2);
  return count;
}
